/*
 ...
 File: ParallelRayTracingWithOpenMP.cpp
 Project: Ray Tracing
 License: https://en.wikipedia.org/wiki/WTFPL
 Author: Modified By Adarsh Koppa Manjunath
 Date: July 30, 2020
 Description:
                This program is wriiten for ray tracing in parallel using OpenMP
                ...
                3) Apply ray tracing algorithm for each pixel ( Parallelized Using OpenMP )
...
bool scene_intersect(const Vec3f& orig, const Vec3f& direction, const std::vector<Sphere>& spheres, Vec3f& hit, Vec3f& N, objecttype& object) {
    ...
    /*
        Step 1: This function is used to find intersection point, we wil walk through each pixel of every sphere and check wethere it will intersect with the ray or not.
                ...
                Return result gives the value for dist_i which is nothing but t in the above formula.
...
Vec3f cast_ray(const Vec3f& orig, const Vec3f& direction, const std::vector<Sphere>& spheres, const std::vector<Light>& lights, size_t depth = 0) {
    ...
    */
    ...
    for (size_t i = 0; i < lights.size(); i++) {
        ...
        Vec3f shadow_orig = light_direction * N < 0 ? point - N * 1e-3 : point + N * 1e-3; // checking if the point lies in the shadow of the lights[i]
...
int main() {
    /*
      1) In this func tion, at very first we declared all the objects ( mostly spheres) and lights.
      2) We create an image of required height and width, and create a 1d buffer for the same
      3) We will walk through every pixel or array element and apply ray tracing algorithm ( parallel)
      4) Finally, computed 1d array is copied to out.ppm file.
   */
...
#pragma omp parallel for  //parallelize the ray tracing algorithm.
    ...
    file_pointer.open("./ParallelRaytracingWithOpenMP.ppm", std::ios::binary);  //image array to out.ppm file
